...
#             axs[j][i].set_xlabel("Feature " + str(i+((i+1)*j)))
#             axs[j][i].set_ylabel("")
#             axs[j][i].set_yticks([])
#         except:
#             print(i+j)
...
for i in range(rows):
    for j in range(cols):
        ...
        try:
            sns.histplot(x=extracted_features.T[j + (i*cols)],ax=axs[i][j], bins=50)
...
plt.savefig("Variational Eagle/Plots/feature_distributions_planar_flows_2_pca", bbox_inches='tight')
plt.show()
