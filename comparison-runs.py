...
# fig, axs = plt.subplots(1, 1, figsize=(20, 10))
#
# default_x = np.array(range(1, radius_correlation.shape[0]+1))
# radius_x = default_x - 0.1
# sersic_x = default_x + 0.1
#
# # axs.scatter(x=range(1, 12), y=sersic_correlation.T[1])
...
# tab20 = plt.get_cmap("tableau-colorblind10")

# axs.scatter(x=range(1, 12), y=sersic_correlation.T[1])
axs.bar(radius_x, radius_correlation.T[1], label="Half-light Radius", width=0.1, yerr=radius_error, capsize=2, lw=1, color=tab20(0), error_kw=dict(alpha=0.3))
# axs.errorbar(x=radius_x, y=radius_correlation.T[1], yerr=radius_error, fmt="o", label="Half-Light Radius", markersize=10, lw=2)

axs.bar(sersic_x, sersic_correlation.T[1], label="Sèrsic Index", width=0.1, yerr=sersic_error, capsize=2, lw=1, color=tab20(1), error_kw=dict(alpha=0.3))
# axs.errorbar(x=sersic_x, y=sersic_correlation.T[1], yerr=sersic_error, fmt="o", label="Sersic Index", markersize=10, lw=2)

axs.bar(axis_ratio_x, axis_ratio_correlation.T[1], label="Axis Ratio", width=0.1, yerr=axis_ratio_error, capsize=2, lw=1, color=tab20(2), error_kw=dict(alpha=0.3))
# axs.errorbar(x=axis_ratio_x, y=axis_ratio_correlation.T[1], yerr=axis_ratio_error, fmt="o", label="Axis Ratio", markersize=10, lw=2)

axs.bar(position_angle_x, position_angle_correlation.T[1], label="Position Angle", width=0.1, yerr=position_angle_error, capsize=2, lw=1, color=tab20(3), error_kw=dict(alpha=0.3))
...
axs.bar(concentration_x, concentration_correlation.T[1], label="Concentration", width=0.1, yerr=concentration_error, capsize=2, lw=1, color=tab20(10), error_kw=dict(alpha=0.3))
...
axs.bar(asymmetry_x, asymmetry_correlation.T[1], label="Asymmetry", width=0.1, yerr=asymmetry_error, capsize=2, lw=1, color=tab20(11), error_kw=dict(alpha=0.3))
# axs.errorbar(x=asymmetry_x, y=asymmetry_correlation.T[1], yerr=asymmetry_error, fmt="o", label="Asymmetry", markersize=10, lw=2)

axs.bar(smoothness_x, smoothness_correlation.T[1], label="Smoothness", width=0.1, yerr=smoothness_error, capsize=2, lw=1, color=tab20(16), error_kw=dict(alpha=0.3))
# axs.errorbar(x=smoothness_x, y=smoothness_correlation.T[1], yerr=smoothness_error, fmt="o", label="Smoothness", markersize=10, lw=2)



axs.set_ylabel("dCor", fontsize=22)
axs.set_xlabel("Principal Components", fontsize=22)
axs.set_xticks(range(1, radius_correlation.shape[0]+1))
axs.tick_params(axis="both", labelsize=22)
axs.legend(fontsize=22, loc='lower center', bbox_to_anchor=(0.5, 1.0), ncol=7)
...
plt.savefig("Variational Eagle/Correlation Plots/Normalising Flow Balanced/PCA/structure_correlation_run_comparison_bars.pdf", bbox_inches='tight')
plt.show()
