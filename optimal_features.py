...
for i in range(1, 17):
    ...
    # feature_loss_1 = np.load("Loss/" + str(i) + "_feature_loss_1.npy")
...
plt.scatter(x=range(1, 17), y=loss, label="Training Images")
plt.scatter(x=range(1, 17), y=val_loss, label="Validation Images")
...
# correlation_df = pd.DataFrame(columns=["Sersic Index", "Axis Ratio", "Semi - Major Axis", "AB Magnitude", "Stellar Mass", "Gas Mass", "Dark Matter Mass", "Black Hole Mass", "Black Hole Subgrid Mass", "Stellar Age", "Star Formation Rate"])
#
# for feature in range(0, len(extracted_features_switch)):
#
#     # create a list to contain the correlation between that feature and each property
